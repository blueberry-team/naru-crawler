---
name: job-search-mid-career
description: Playwright MCP로 일본 중도채용 페이지를 크롤링하고, 명시된 정보만 추출한 뒤, 사용자 노출 텍스트를 일본어에서 한국어로 번역하고, 공고별로 NARU jobs API에 DRAFT payload를 제출한다.
...
이 스킬은 일본 중도채용 페이지를 크롤링해서 NARU에 DRAFT 공고로 적재할 때 사용한다.
...
```text
/job-search-mid-career "https://example.com/recruit/career" hitachi
```
...
3. [`references/enums.md`](./references/enums.md)
4. [`references/translation-glossary.md`](./references/translation-glossary.md)
5. [`rules/01-navigation.md`](./rules/01-navigation.md)
6. [`rules/02-classification.md`](./rules/02-classification.md)
7. [`rules/03-join-date.md`](./rules/03-join-date.md)
8. [`rules/04-salary.md`](./rules/04-salary.md)
9. [`rules/05-target-audience.md`](./rules/05-target-audience.md)
10. [`rules/06-selection-process.md`](./rules/06-selection-process.md)
11. [`rules/07-content-fields.md`](./rules/07-content-fields.md)
12. [`rules/08-target-candidate.md`](./rules/08-target-candidate.md)
...
- enum, 숫자, URL, slug 는 그대로 둔다.
- 모든 공고는 `publishStatus: "DRAFT"` 로 제출한다.

## 필수 워크플로우

1. 시작 URL을 Playwright MCP로 연다.
2. 중도채용 동선을 따라간다. 외부 ATS 페이지라도 상세 공고가 있으면 계속 진행한다.
