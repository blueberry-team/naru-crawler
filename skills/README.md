...
| 순서 | 스킬 | 역할 |
|------|------|------|
| 1 | [`review-draft`](review-draft/) | 동일한 채용공고 원본을 크롤링하여 25개 기준 Deep Review 진행 |
| 2 | [`review-and-fix`](review-and-fix/) | 리뷰에서 발견된 ⚠️/❌ 항목 수정 (PUT API) + 재리뷰 |
| 3 | [`cleanup-deadline`](cleanup-deadline/) | 마감일 경과 / 원본 삭제 공고 정리 (isDeadlinePassed / unpublish) |

**흐름**:
1. **리뷰**: 공고의 source URL을 WebFetch → 원본과 DB 데이터 25개 항목 대조
2. **수정**: 분류 오류(position/locations), 연봉 오차, 텍스트 품질 등 자동 수정
3. **마감 정리**: deadline 경과 → `isDeadlinePassed=true`, 원본 404 → `unpublish`
4. 모든 결과를 `history/` 폴더에 기록

**실행 예시**:
```text
DRAFT 공고 5개 샘플링해서 리뷰하고 문제 있으면 수정해줘
