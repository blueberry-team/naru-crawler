...
- 신입 공고인가
- 경력직 / 아르바이트 / 계약직 / 비신입 공고가 섞이지 않았는가
...
- `【番号】` prefix 를 제거했는가
- `position` 이 6개 graduate enum 중 하나인가
- `experienceLevel` 이 `NEW_GRAD` 인가
...
- 졸업 시기 규칙을 적용할 수 있으면 적용했는가
- 그래도 불명확하면 `未定` 으로 넣었는가
...
- `targetAudience` 에 졸업 시기 / 졸업 연도 / 기졸 가능만 들어갔는가
