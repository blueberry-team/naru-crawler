...
모든 Fix 액션은 `history/` 폴더의 해당 Job 섹션에 기록한다.

### 기록 내용
- **수정 전후 비교 테이블**: 항목, 수정 전 값, 수정 후 값, API 응답 코드
- **재평가 결과**: 수정된 항목의 ⚠️→✅ 등 변경
- **HOLD 목록**: 수정 불가 건은 별도 정리

### 기록 위치
- Deep Review가 이미 기록된 `history/historyXXX-YYY.md` 파일의 해당 Job 아래에 `[RE-REVIEW]` 섹션으로 추가
- HOLD 목록은 파일 말미에 별도 섹션

### 기록 시점
- PUT 실행 직후 즉시 기록 (PUT 전에 변경될 값을 먼저 로그)
- 재평가 완료 후 종합 점수 업데이트
