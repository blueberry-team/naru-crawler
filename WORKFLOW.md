...
> 모든 적재는 사용자의 검토·승인 후 단건(1개)씩 진행.
...
| 공고 수정 (PUT) | 사용자 허락 받은 후에만 |
| 공고 삭제 (DELETE) | 직접 실행 절대 금지 — 반드시 사용자에게 먼저 물어볼 것 |
| 기업 생성 | slug 확인 후 없을 때만, 사용자 보고 후 |
| 기업 수정/삭제 | 절대 금지 |
...
`MISTAKES.md` 를 반드시 먼저 읽고 동일 실수 반복 방지.

**주요 반복 금지 실수:**
- [ ] 기업 slug 확인 없이 새 회사 생성 (DB 오염 원인)
- [ ] `publishStatus` 아닌 `status` 필드명 사용 → 자동 PUBLISHED됨
- [ ] 지원서 페이지 URL을 `jobSourceUrl`로 사용 (상세 페이지 URL을 써야 함)
- [ ] 제목에 불필요한 prefix(번호, 코드 등) 그대로 남김
- [ ] `salaryMin`/`salaryMax` null로 방치 (공개 데이터 조사 없이 포기)
- [ ] enum 실제 배포 버전 확인 없이 추측
- [ ] 승인 없이 혼자 적재 실행
- [ ] 일본어 원문 그대로 적재
...
### 출처 URL
- 공고 **상세 페이지** URL을 `jobSourceUrl`로 사용
- 지원서 페이지, 목록 페이지 등은 사용 금지

### 날짜 확인 항목
| 항목 | 확인 방법 | 우선순위 영향 |
|------|----------|------------|
| **마감일** (deadline) | 원문 페이지에서 탐색 | 마감 임박 → 높은 우선순위 |
| **게재일** (공고 등록일) | API 응답 또는 페이지 내 날짜 | 최신 공고 우선 |
| **수시 채용 여부** | 원문 "随時", "通年" 등 | deadline null 처리 |
...
### 급여 추정 기준
- 기업 공식 채용 페이지, OpenWork, 취업 에이전시 등 공개 데이터 참고
- 확신이 없으면 `salaryMin`만 보수적으로 설정, `salaryMax`는 null
- 완전히 불명확하면 둘 다 null
...
- 동일 수정이 크롤러 로직에도 해당하면 → 코드 수정 + `MISTAKES.md` 업데이트 + commit/push
...
□ 4. 원문↔번역 대조 + 전체 payload → 사용자에게 보고
